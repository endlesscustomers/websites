function sizeSectionTestimonialColumnsDividers() {
	setTimeout(function () {
		const sections = document.querySelectorAll('.section--testimonialcolumns--hasdividers')

		sections.forEach(function (thisSection) {
			const firstColumn = thisSection.querySelector('.section--testimonialcolumns--column:nth-child(1)')
			const thisSectionColumnVerticalPadding = (firstColumn.offsetHeight - firstColumn.clientHeight) / 2

			const dividers = thisSection.querySelectorAll('.section--testimonialcolumns--divider--vertical')
			dividers.forEach((divider) => {
				divider.style.height = ''
				divider.style.margin = ''
			})

			const columnBodies = thisSection.querySelectorAll('.section--testimonialcolumns--column--body, .section--coulumns--column--body--content')
			...
			let rowHeights = []
			let contentHeights = []
			let totalColumns = thisSection.querySelectorAll('.section--testimonialcolumns--column').length
			let columnWidths = {}
			const thisColumns = thisSection.querySelector('.section--testimonialcolumns--grid')
			...
			const columns = thisSection.querySelectorAll('.section--testimonialcolumns--column')
			columns.forEach(function (column, index) {
				const thisHeight = column.querySelector('.section--testimonialcolumns--column--body').offsetHeight
				const contentHeight = column.querySelector('.section--testimonialcolumns--column--body--content').offsetHeight
				let rowIndex
			...
			columns.forEach(function (column, index) {
				...
				const body = column.querySelector('.section--testimonialcolumns--column--body')
...
document.addEventListener('DOMContentLoaded', function () {
	sizeSectionTestimonialColumnsDividers()
})
