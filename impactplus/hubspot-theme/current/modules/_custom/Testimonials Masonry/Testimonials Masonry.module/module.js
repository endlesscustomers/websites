function sizeSectionTestimonialsMasonryDividers() {
	setTimeout(function () {
		const sections = document.querySelectorAll('.section--testimonialsmasonry--hasdividers')

		sections.forEach(function (thisSection) {
			const firstColumn = thisSection.querySelector('.section--testimonialsmasonry--column:nth-child(1)')
			const thisSectionColumnVerticalPadding = (firstColumn.offsetHeight - firstColumn.clientHeight) / 2

			const dividers = thisSection.querySelectorAll('.section--testimonialsmasonry--divider--vertical')
			dividers.forEach((divider) => {
				divider.style.height = ''
				divider.style.margin = ''
			})

			const columnBodies = thisSection.querySelectorAll('.section--testimonialsmasonry--column--body, .section--coulumns--column--body--content')
			...
			let rowHeights = []
			let contentHeights = []
			let totalColumns = thisSection.querySelectorAll('.section--testimonialsmasonry--column').length
			let columnWidths = {}
			const thisColumns = thisSection.querySelector('.section--testimonialsmasonry--grid')
			...
			const columns = thisSection.querySelectorAll('.section--testimonialsmasonry--column')
			columns.forEach(function (column, index) {
				const thisHeight = column.querySelector('.section--testimonialsmasonry--column--body').offsetHeight
				const contentHeight = column.querySelector('.section--testimonialsmasonry--column--body--content').offsetHeight
				let rowIndex
			...
			columns.forEach(function (column, index) {
				...
				const body = column.querySelector('.section--testimonialsmasonry--column--body')
...
document.addEventListener('DOMContentLoaded', function () {
	sizeSectionTestimonialsMasonryDividers()
})
